...
static void test_gs_delassus_vgap(void) {
    ...
    b3_prepare_joints(&artw, 1.0f / 60.0f);
    b3_prepare_joints(&indw, 1.0f / 60.0f);
    b3_gs_load(&artw, abl, ajl, acl);
    b3_gs_load(&indw, ibl, ijl, icl);
    seed_tip_contact(&acl[0], 0, tip);
    seed_tip_contact(&icl[0], 0, tip);
    float nm = kn > 0.0f ? 1.0f / kn : 0.0f;
    acl[0].points[0].normal_mass = nm;
    icl[0].points[0].normal_mass = nm;

    artw.contact_count = 1;
    for (int i = 0; i < artw.joint_count; i++) {
        b3_cache_revolute_gs(&ajl[i], &artw.joints[i],
            &abl[ajl[i].body_a], &abl[ajl[i].body_b]);
    }
    B3TreeFactor f;
    CHECK(b3_tree_factor(&artw, abl, ajl, 1, &f));
    CHECK(b3_tree_solve(&artw, abl, ajl, acl, 1.0f / 60.0f, 60.0f, 1));
    ...
    printf("  tree art mid_wz=%g tip_vy=%g vgap0=%g vgap1=%g\n",
        artw.bodies[1].ang_vel.z, artw.bodies[tip].lin_vel.y, va0, va1);
    printf("  tree ind mid_wz=%g tip_vy=%g vgap0=%g vgap1=%g nimp=%g/%g\n",
    ...
    CHECK(isfinite(artw.bodies[1].ang_vel.z));
#if B3_ART_CONTACTS
...
#else
    /* Independent 1/m then tree project. Contacts must still fire. */
    CHECK(acl[0].points[0].normal_impulse > 0.0f);
    CHECK(va0 < 2.0e-2f);
    CHECK(va1 < 2.0e-2f);
#endif
}
...
static void test_gs_step_drop(void) {
    ...
    printf("  tree-step drop com.y=%g hinge=%g contacts=%d\n",
...
static void test_gs_grain_rest(void) {
    ...
    printf("  tree grain y=%g vy=%g contacts=%d\n",
...
static void test_gpu_gs_drop(void) {
    ...
    printf("  gpu-tree n=%d com.y=%g hinge=%g\n",
...
int main(void) {
    ...
    printf("tree-delassus: OK\n");
    test_gs_step_drop();
    printf("tree-step: OK\n");
    test_gs_grain_rest();
    printf("tree-grain: OK\n");
    test_gs_rolling();
    printf("tree-rolling: OK\n");
#ifdef __CUDACC__
    test_gpu_gs_drop();
    printf("gpu-tree: OK\n");
#endif
